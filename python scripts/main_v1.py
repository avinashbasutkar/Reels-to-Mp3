...
if access_token and mp3_content:
    ...
    files = {
        "track[title]": (None, "Instagram Reel MP3"),
...
# Step 4: Update SoundCloud playlist with new track
playlist_id = 2076895419
if access_token and track_id:
    # Get current playlist tracks
    playlist_url = f"https://api.soundcloud.com/playlists/{playlist_id}"
    playlist_headers = {
        "accept": "application/json; charset=utf-8",
        "Authorization": f"OAuth {access_token}"
    }
    ...
    current_tracks = playlist_response.get("tracks", [])
    # Build new tracks list with the new track appended
    updated_tracks = [{"id": t["id"]} for t in current_tracks if "id" in t]
    print(f"Current number of tracks in playlist: {len(updated_tracks)})")
    print(f"updated_tracks before adding new track: {updated_tracks}")
    updated_tracks.append({"id": track_id})
    print(f"updated_tracks after adding new track: {updated_tracks}")
    print(f"Updated number of tracks in playlist: {len(updated_tracks)})")
    # Prepare payload for playlist update
    update_payload = {
        "playlist": {
    ...
    r = requests.put(playlist_url, headers=update_headers, json=update_payload)
    update_response = r.json()
    print("Playlist update response:", update_response)
elif not track_id:
    print("No track ID to add to playlist.")